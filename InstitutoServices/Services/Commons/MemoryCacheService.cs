...
namespace InstitutoServices.Services.Commons
{
    public class MemoryCacheService
    {
        ...
        public async Task<List<T>> GetAllCache<T>(string key) where T : class, IEntityWithId
        {
        ...
        public async Task<bool> DeleteCache<T>(int id, string key) where T : class, IEntityWithId
        {
            IGenericService<T> genericService = new GenericService<T>();
            var result = await genericService.DeleteAsync(id);
            if (result)
            {
                var list = await GetAllCache<T>(key);
        ...
        public async Task<T> AddCache<T>(T entity, string key) where T : class, IEntityWithId
        {
            IGenericService<T> genericService = new GenericService<T>();
        ...
        public async Task<bool> UpdateCache<T>(T entity, string key) where T : class, IEntityWithId
        {
            IGenericService<T> genericService = new GenericService<T>();
